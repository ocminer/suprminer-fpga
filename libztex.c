...
static uint64_t libztex_timeval_elapsed_us(const struct timeval *start,
	const struct timeval *end)
{
	int64_t elapsed_us = (int64_t)(end->tv_sec - start->tv_sec) * 1000000LL +
		(int64_t)end->tv_usec - (int64_t)start->tv_usec;
	return elapsed_us > 0 ? (uint64_t)elapsed_us : 0;
}

int libztex_sendDataEx(struct libztex_device *ztex, unsigned char *sendbuf,
	int len, struct libztex_send_trace *trace)
{
	int cnt = 0;
	int idx = 0;
	...
	while (len > 0) {
		...
		if (trace) {
			unsigned slot = trace->stored;
			gettimeofday(&call_end, NULL);
			trace->calls++;
			if (slot < LIBZTEX_SEND_TRACE_MAX) {
				trace->requested[slot] = chunk;
				trace->returned[slot] = cnt;
				trace->duration_us[slot] =
					libztex_timeval_elapsed_us(&call_start, &call_end);
				trace->stored++;
			} else {
				trace->overflow = true;
			}
	...
	if (trace) {
		...
		trace->total_us =
			libztex_timeval_elapsed_us(&total_start, &total_end);
	}

	return cnt;
}

int libztex_sendData(struct libztex_device *ztex, unsigned char *sendbuf,
	int len)
{
	return libztex_sendDataEx(ztex, sendbuf, len, NULL);
}

int libztex_sendFrameExact(struct libztex_device *ztex,
	const unsigned char *frame, int len, unsigned timeout_ms)
{
	int count;

	if (!ztex || !ztex->hndl || !frame || len < 1 || len > 64)
		return LIBUSB_ERROR_INVALID_PARAM;
	count = libusb_control_transfer(ztex->hndl, 0x40, 0x80, 0, 0,
		(unsigned char *)frame, (uint16_t)len, timeout_ms);
	if (count < 0)
		return count;
	return count == len ? count : LIBZTEX_ERROR_SHORT_TRANSFER;
}

int libztex_readFrameExact(struct libztex_device *ztex,
	unsigned char *frame, int len, unsigned timeout_ms)
{
	...
	if (!ztex || !ztex->hndl || !frame || len < 1 || len > 64)
		return LIBUSB_ERROR_INVALID_PARAM;
	count = libusb_control_transfer(ztex->hndl, 0xc0, 0x81, 0, 0,
		frame, (uint16_t)len, timeout_ms);
	if (count < 0)
		return count;
	return count == len ? count : LIBZTEX_ERROR_SHORT_TRANSFER;
}

int libztex_readDataEx(struct libztex_device *ztex, uint32_t *nonce,
	uint32_t *hash7, uint32_t *golden, unsigned char raw[16],
	uint64_t *duration_us)
{
	unsigned char rbuf[16] = { 0 };	// Stores GN1, Nonce, Hash, GN2
	int cnt;
	...
	/* The FPGA serves these 16 bytes from a walking read pointer. Firmware
	 * request 0x81 pulses wr_start for every control read, so each fresh
	 * request restarts at byte zero. Never stitch a short transfer to a
	 * second request: that would combine two different snapshots. Demand
	 * all 16 bytes in one transfer and let the caller choose the retained
	 * legacy re-select retry or the explicit no-reselect experiment. */
	cnt = libusb_control_transfer(ztex->hndl, 0xc0, 0x81, 0, 0, rbuf, 16, 1000);
	if (duration_us) {
		gettimeofday(&tv1, NULL);
		*duration_us = libztex_timeval_elapsed_us(&tv0, &tv1);
	}
	if (raw && cnt > 0)
		memcpy(raw, rbuf, cnt < 16 ? (size_t)cnt : sizeof(rbuf));
	if (unlikely(cnt < 0)) {
		applog(LOG_ERR, "%s: Unable To Read ZTEX (Error: %d)", ztex->repr, cnt);
		return cnt;
	}
	if (unlikely(cnt != 16)) {
		/* Keep the legacy diagnostic text stable for the default path. */
...
int libztex_readData(struct libztex_device *ztex, uint32_t *nonce,
	uint32_t *hash7, uint32_t *golden)
{
	return libztex_readDataEx(ztex, nonce, hash7, golden, NULL, NULL);
}
