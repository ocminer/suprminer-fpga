...
static void draw(void)
{
	...
	for (i = 0; i < g_nboards && y < dev_area_bot; i++) {
		...
		for (j = 0; j < f->ztex_info->numberOfFpgas; j++, rownum++) {
			...
			char ago[32]; fmt_ago(ago, sizeof(ago), &s->last_share_tv);
			const char *dev = (j == 0) ? f->name : "";
			/* Exact descriptor clock supports rational fixed images such as
			 * 99.692307 MHz; legacy operation falls back to the CLI display. */
			double clk = s->hash_clock_mhz > 0.0 ?
				s->hash_clock_mhz : g_hash_clock_mhz;
			snprintf(buf, sizeof(buf),
